...
class Bluetoothctl:
	...
	def _start_bluetoothctl(self):
		...
		if not self.isReady:
			print(f"Unable to start bluetoothctl after {self.max_attempts} attempts.")

	def send(self, command, pause=0):
		if not self.isReady or self.process is None:
			return
		self.process.send(f"{command}\n")
		sleep(pause)
		try:
			if self.process.expect(["#", EOF, TIMEOUT], timeout=5):
				raise Exception(f"bluetoothctl failed after {command}")
		except TIMEOUT:
			print(f"Timeout while executing command: {command}")
		except EOF:
			print(f"EOF reached while executing command: {command}")
		except Exception as e:
			raise e
	...
	def pair(self, mac_address):
		...
		try:
			...
			res = self.process.expect(["Failed to pair", "Pairing successful", "Passkey: ", "PIN code: ", "Request authorization", EOF], timeout=15)
			if res == 1:
				return True
			elif res == 4:
				self.send("yes")
				...
				if mac_address in [x['mac_address'] for x in self.get_paired_devices()]:
					return True
				res = self.process.expect(["Request confirmation", EOF], timeout=10)
				return res == 0
			elif res in [2, 3]:
	...
	def remove(self, mac_address):
		...
		try:
			...
			res = self.process.expect(
				["not available", "Device has been removed", EOF], timeout=5
			)
			return res == 1
	...
	def connect(self, mac_address):
		...
		try:
			...
			res = self.process.expect(
				["Failed to connect", "Connection successful", EOF], timeout=10
			)
			return res == 1
	...
	def disconnect(self, mac_address):
		...
		try:
			...
			res = self.process.expect(
				["Failed to disconnect", "Successful disconnected", EOF], timeout=10
			)
			return res == 1
